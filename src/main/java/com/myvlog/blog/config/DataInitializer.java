...
public class DataInitializer implements CommandLineRunner {
    ...
    private void initDefaultAnnouncements() {
        try {
            Long count = jdbcTemplate.queryForObject("SELECT count(*) FROM announcements", Long.class);
            if (count != null && count == 0) {
                log.info("Initializing default announcements...");
                jdbcTemplate.execute("""
                    INSERT INTO announcements (title, content, type, is_active, created_at, updated_at)
                    VALUES 
                    ('欢迎来到 MyVlog', '欢迎访问我的个人博客系统！这里是我分享生活和技术的地方。', 'general', 1, NOW(), NOW()),
                    ('系统版本更新 v1.0', '我们的博客系统迎来了第一个正式版本更新，修复了若干已知问题。', 'system_update', 1, NOW(), NOW())
                """);
                log.info("Default announcements initialized.");
            }
        } catch (Exception e) {
            log.warn("Failed to init announcements: " + e.getMessage());
        }
    }
